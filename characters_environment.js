...
function rage() {
  Games = "start";
  document.getElementById("ragestatus").innerHTML = "Prepare to start raging.";
...
function game(){
  ...
  if(gameConfig.status==='start'){
    ...
    textSize(40);
    textAlign(CENTER);
    text("Just start the rage game by pressing arrow keys 🔥", gameConfig.screenX/2, gameConfig.screenY/2);
    textSize(40);
...
function changeGameStatud(character){
  if(Games=="start" && nosex!="" && gameConfig.status==="start") {
    world_start.play();
